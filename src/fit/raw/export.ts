...
import { patchFileIdInPlace, spliceLaps, type RawDeviceChange } from './patch';
...
function lapsChangedBetween(original: FitModel, current: FitModel): boolean {
 ...
 * file: in-place file_id rewrite plus lap splicing when the lap layout
 * changed. Everything the user didn't edit — including the proprietary
 * messages and fields no SDK understands — is emitted byte-for-byte as the
 * device wrote it. A zero-edit export is byte-identical to the input.
 * (Full decode→re-encode is not an option: Garmin Connect rejects
 * @garmin/fitsdk encoder output wholesale; see parse.ts.)
 */
export function exportEditedFit(originalBytes: Uint8Array, originalModel: FitModel, currentModel: FitModel): Uint8Array {
  ...
  const lapsChanged = lapsChangedBetween(originalModel, currentModel);

  if (!device && !lapsChanged) {
